...
| Account-switch guards | `MainViewModel.OnMt5AccountSwitched`, run by the login dialog's OnSignedIn continuation after every **successful** MT5 login (a refused login changes nothing) | (1) Stops the FX brain — its open positions, exposure cap and supervisor baseline belong to the account that was just signed out, so no engine cycle may fire against the new one; (2) resets the `ManualRealMoneyGate` session unlock, which was armed for the previous account (a fresh account starts locked; only app close reset it before). Journals the action as `MT5_SESSION` | Fail safe: both guards are best-effort and can never mask the successful login; the stop path is the same `ShutdownFxBrain` teardown the app shutdown uses |
...
|---|---|
| `ChartMenuWiringTests` (DongGfx.App) | The chart trading menu's wiring rails: the open is deferred through the openMenu callback (a synchronous open inside the right-click route is dismissed by that same input sequence — regression for the live-only menu bug), the menu carries Buy/Sell at ticket lots/Add-level/Clear, the Add-level item renders the horizontal level at the clicked price, Clear empties the canvas, and Buy/Sell target the ticket symbol — the guarded order path itself stays covered by the chart-trading suite above. |
| `UiaSmokeTests` (DongGfx.App, CI's uia-smoke job) | Launches the real WPF shell and asserts via UI Automation that the Terminal tab renders: tab select, MARKET WATCH header, FX-brain badge, candles panel caption, and the account bar's settle — live login/rows with a sidecar up, the "bridge offline" degradation with it down. Never kills a running instance (attaches via TF_UIA_PID or skips vacuously). Plumbing smoke only: no gate-state assertions, so it carries no RealMoney trait. |
