...
|---|---|---|---|
| `gate-drill` | Runs the real-money gate test suite on the tagged commit: Core's decision/parse matrix, plus the App-level E2E lifecycle — locked start refused → unlock arms → mid-session stop on settlement re-check → hub drops without a restart ladder → manual surfaces fail closed. All against the fake broker: no network, no real funds. | The gate is broken **or flaky on the exact commit being shipped**. An alert issue auto-files on `ci-gate-drill` with a log excerpt. | **Do not ship.** Fix on `main` via PR, merge, then tag again (see rollback below — never "re-run until green"). |
