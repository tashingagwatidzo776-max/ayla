...
| Tag | What happened |
|---|---|
| `v0.0.9` | Per-account tokens & the Monday open watcher (#88): an Accounts-tab guided flow detects a shared PAT, verifies each fresh token over Deriv discovery (plain HTTPS — live sessions untouched) and re-imports accounts in place (ids preserved); `open-watcher.ps1` plus a registered Windows task toast on the week's first BRAIN_DECISION/settlement, then record soak evidence and run ci-watch before standing down. Chain green first-run on `3b78b2f`; asset attached and stamp-verified on the first dispatch. Soak evidence refreshed (73 entries, CLEAN). |
| `v0.0.8` | Completed the rebrand at the code level (#86): `DongGfx.sln`, `DongGfx.App/Core/Deriv` (+ tests), namespaces and the shipped binary now `DongGfx.exe`; scripts, workflows, docs and the safety-audit tables renamed with them. Data paths under the app's application-data directory deliberately unchanged, so accounts, tokens and journals carried over with no re-import. Chain green first-run on `86254c8` from the renamed tree; asset attached and stamp-verified on the first dispatch. |
