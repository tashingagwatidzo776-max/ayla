...
| Sat 02:37 | [Real-money gate drill](.github/workflows/gate-drill.yml) | the full real-money gate lifecycle against the fake broker — every `Category=RealMoney` test class (gate decisions, unlock arming/audit, manual surfaces, digest arm leg, hub + mid-session E2E); release tags (`v*`) re-rehearse it before anything ships, and failures file on `ci-gate-drill` |
