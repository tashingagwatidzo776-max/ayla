...
/// <summary>
/// Architecture boundary test: Tf.Core is the headless core (brains, stores,
/// analytics, plumbing) and must never reference UI assemblies — the WPF
/// surface lives in Tf.App. Headless services extracted from view models
...
/// build the moment Tf.Core pulls in a UI framework again.
/// </summary>
[Trait("Category", "Unit")]
public class ArchitectureTests
{
    ...
    [Fact]
    public void TfCore_DoesNotReferenceUiAssemblies()
    {
        var referenced = ReadAssemblyReferences(typeof(Tf.Core.Brain.GrowthPlan).Assembly.Location);
        ...
        Assert.True(
            violations.Count == 0,
            "Tf.Core must not reference UI assemblies, but it references: " +
            string.Join(", ", violations) +
            ". Move the UI-dependent code into Tf.App (or extract the headless logic).");
    }
    ...
    public void MetadataScan_IsLive_SeesCoreReferences()
    {
        ...
        // boundary test above would pass vacuously. Tf.Core always references
